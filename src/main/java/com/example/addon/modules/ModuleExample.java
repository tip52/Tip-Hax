...
public class ModuleExample extends Module {
    ...
    private boolean lastChatFeedback = false;
    @EventHandler
    private void onRender3d(Render3DEvent event) {
        // Create & stretch the marker object


 boolean current = chatFeedback.get();
    if (current != lastChatFeedback) {
        if (current) {
           StringBuilder message = new StringBuilder(" it on ");
            ChatUtils.sendMsg(Text.of(message.toString()));
          
        } else {
          StringBuilder message = new StringBuilder(" it off ");
            ChatUtils.sendMsg(Text.of(message.toString()));
        }
        lastChatFeedback = current;
    }
